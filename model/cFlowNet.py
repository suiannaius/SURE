...
from model.unet_blocks2D import *
from model.unet2D import Unet
from model.utils2D import init_weights,init_weights_orthogonal_normal, l2_regularisation
import torch.nn.functional as F
from torch.distributions import Normal, Independent, kl
import pdb
from model import flows
from model.nflib.flows import *
from model.cflow_layers import *
...
class Encoder(nn.Module):
    ...
    """
    def __init__(self, input_channels, num_filters, no_convs_per_block, initializers, num_classes, padding=True, posterior=False,norm=False):
    ...
    #         print(f"Layer {i} input stats - min:", input.min().item(), "max:", input.max().item())
    #         input = layer(input)
    #         print(f"Layer {i} output stats - min:", input.min().item(), "max:", input.max().item())
    #         if torch.isnan(input).any():
    #             print(f"NaN detected at layer {i}: {layer}")
    #             break
    #     return input
...
class AxisAlignedConvGaussian(nn.Module):
    ...
    def __init__(self, input_channels, num_filters, no_convs_per_block, latent_dim, 
            initializers, num_classes, posterior=False,norm=False):
        ...
        self.encoder = Encoder(self.input_channels, self.num_filters, self.no_convs_per_block, initializers, 
                num_classes, posterior=self.posterior,norm=norm)
...
class glowDensity(nn.Module):
    ...
    def __init__(self, num_flows, input_channels, num_filters, no_convs_per_block, 
            latent_dim, initializers, num_classes, posterior=False,norm=False):
        super(glowDensity, self).__init__()
    
        self.base_density = AxisAlignedConvGaussian(input_channels, num_filters, 
                no_convs_per_block, latent_dim, initializers, num_classes, posterior=True,norm=norm).to(device)
...
class cFlowNet(nn.Module):
    ...
    def __init__(self, input_channels=1, num_classes=1, num_filters=[32,64,128,256], 
        ...
        self.prior = AxisAlignedConvGaussian(self.input_channels, self.num_filters, 
                self.no_convs_per_block, self.latent_dim,  self.initializers,num_classes, norm=norm).to(device)

        if flow:
            if glow:
                self.posterior = glowDensity(self.flow_steps, self.input_channels, self.num_filters, self.no_convs_per_block, 
                    self.latent_dim, self.initializers,num_classes,posterior=True,norm=norm).to(device)
            else:
            ...
            self.posterior = AxisAlignedConvGaussian(self.input_channels, self.num_filters, 
                    self.no_convs_per_block, self.latent_dim, self.initializers, num_classes, posterior=True,norm=norm).to(device)
...
#        pdb.set_trace()
    ...
    def sample_m(self, img,m,testing=False): # change by coco
        ...
        if testing == False:
            ...
            res = torch.ones(img_num, m, self.num_classes, h, w).cuda()
