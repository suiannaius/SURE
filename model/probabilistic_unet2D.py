...
class Encoder(nn.Module):
    ...
    def __init__(self, input_channels, num_filters, no_convs_per_block, initializers, num_classes, padding=True, posterior=False):
        ...
        if posterior:
            ...
            self.input_channels += num_classes # changed by coco input_channels + segm_channels
...
class ProbabilisticUnet2D(nn.Module):
    """
    ...
    def __init__(self, input_channels=1, num_classes=1, num_filters=[32, 64, 128, 192], latent_dim=6, no_convs_fcomb=4,
                 beta=10.0):
        super(ProbabilisticUnet2D, self).__init__()
        self.input_channels = input_channels
        self.num_classes = num_classes
        self.num_filters = num_filters
        self.latent_dim = latent_dim
        ...
        self.prior = AxisAlignedConvGaussian(self.input_channels, self.num_filters, self.no_convs_per_block,
                                             self.latent_dim, self.initializers, num_classes).to(device)
        self.posterior = AxisAlignedConvGaussian(self.input_channels, self.num_filters, self.no_convs_per_block,
                                                 self.latent_dim, self.initializers, num_classes, posterior=True).to(device)
