...
def run_inference(project_name, consider_spacing=False, method='base'):
    ...
    log_dir = f'./results/writer/inference_{project_name}'
    ...
    fold_dices = {f'fold_{i}': {} for i in range(num_fold)}
    fold_assds = {f'fold_{i}': {} for i in range(num_fold)}
    fold_hd95s = {f'fold_{i}': {} for i in range(num_fold)}
    fold_ueos = {f'fold_{i}': {} for i in range(num_fold)}
    fold_eces = {f'fold_{i}': {} for i in range(num_fold)}
    if method == 'base':
        task_info = {
        'Timestamp': datetime.now().strftime('%Y-%m-%d %H:%M:%S,%f'),
        'Project Name': project_name,
        'Backbone': f"{args.backbone}Net",
        ...
        'Writer': f"tensorboard --logdir=./results/writer/inference_{project_name}"
        }
    else:
        task_info = {
        'Timestamp': datetime.now().strftime('%Y-%m-%d %H:%M:%S,%f'),
    ...
    for fold in range(num_fold):
        ...
        dice_RV, dice_Myo, dice_LV, assd_RV, assd_Myo, assd_LV, HD95_RV, HD95_Myo, HD95_LV, \
            ECE_RV, ECE_Myo, ECE_LV, UEO_RV, UEO_Myo, UEO_LV = inference(model=model,
    ...
    dice_RVs = [fold_dices[f'fold_{i}']['RV'] for i in range(num_fold)]
    dice_Myos = [fold_dices[f'fold_{i}']['Myo'] for i in range(num_fold)]
    dice_LVs = [fold_dices[f'fold_{i}']['LV'] for i in range(num_fold)]
    ...
    assd_RVs = [fold_assds[f'fold_{i}']['RV'] for i in range(num_fold)]
    assd_Myos = [fold_assds[f'fold_{i}']['Myo'] for i in range(num_fold)]
    assd_LVs = [fold_assds[f'fold_{i}']['LV'] for i in range(num_fold)]
    ...
    hd95_RVs = [fold_hd95s[f'fold_{i}']['RV'] for i in range(num_fold)]
    hd95_Myos = [fold_hd95s[f'fold_{i}']['Myo'] for i in range(num_fold)]
    hd95_LVs = [fold_hd95s[f'fold_{i}']['LV'] for i in range(num_fold)]
    ...
    ueo_RVs = [fold_ueos[f'fold_{i}']['RV'] for i in range(num_fold)]
    ueo_Myos = [fold_ueos[f'fold_{i}']['Myo'] for i in range(num_fold)]
    ueo_LVs = [fold_ueos[f'fold_{i}']['LV'] for i in range(num_fold)]
    ...
    ece_RVs = [fold_eces[f'fold_{i}']['RV'] for i in range(num_fold)]
    ece_Myos = [fold_eces[f'fold_{i}']['Myo'] for i in range(num_fold)]
    ece_LVs = [fold_eces[f'fold_{i}']['LV'] for i in range(num_fold)]
    ...
    all_results = {**task_info, **seg_info}
    json_path = f'./results/inference/ACDC/{project_name}.json'
    os.makedirs(os.path.dirname(json_path), exist_ok=True)
    with open(json_path, 'w', encoding='utf-8') as f:
        json.dump(all_results, f, indent=4, separators=(',', ': '), ensure_ascii=False)
    print(f'Done! Results have been saved in {json_path}.')
